...
def load_model(path: Path, device: torch.device) -> SpectrogramGenerator:
    model = None
    if path.suffix == ".nemo":
        # Load FastPitchModel instead of SpectrogramGenerator
        model = FastPitchModel.restore_from(path, map_location=device)
    elif path.suffix == ".ckpt":
        # Load FastPitchModel from checkpoint
        model = FastPitchModel.load_from_checkpoint(path, map_location=device)
    else:
